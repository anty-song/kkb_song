...
$(function() {
    // 检测是否登录
    var UC_HOST = 'http://' + location.host.replace('www', 'uc2login');

    try{
        var user = $.cookie('user');
        if(!user || (user && !$.parseJSON(user).token)) {
            var checkLogin = UC_HOST + '/sso/token';
            $.ajax({
                url: checkLogin,
                type: 'GET',
                dataType: 'jsonp',
                success: function(data) {
                    if(data.success) {
                        ...
                        $.ajax({
                            url: 'http://' + location.host.replace('www', 'student') + '/api/users/lback',
                            type: 'GET',
                            data: params,
                            success: function(res) {
                                if(res.error_code == 0) {
                                    var user = stringify(res.data);
                                    $.cookie('user', user, {
                                        domain: '.kaikeba.com',
                                        path: '/'
                                    });
                                    ...
                                    if(res.error_code == 51005) {
                                        // alert('该用户未在此网站注册，请联系管理员。');
                                    }
                                }
                            }
                        })
